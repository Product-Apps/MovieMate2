...
const wordAssociations: string[] = [
  'Adventure', 'Calm', 'Exciting', 'Romantic', 'Thrilling',
  'Funny', 'Mysterious', 'Epic', 'Dramatic', 'Light-hearted'
];

const moodColors: string[] = ['#3B82F6', '#9333EA', '#F97316', '#EF4444', '#10B981', '#EC4899', '#14B8A6', '#F59E0B'];
...
export default function MoodScreen() {
  ...
  const [moodSlider, setMoodSlider] = useState<number>(50);
  const [selectedColor, setSelectedColor] = useState<string>('#3B82F6');

  const GameCard = ({ title, description, icon, gameId, isSelected, onSelect }: GameCardProps) => {
    return (
      <TouchableOpacity
  ...
  const saveMood = async (): Promise<void> => {
    ...
    let history: Array<{ mood: string; date: string; movieCount: number }> = historyStr ? JSON.parse(historyStr) : [];
    history.unshift({
      mood,
    ...
    <View style={[styles.container, { backgroundColor: colorScheme === 'dark' ? '#000' : '#fff' }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <GameCard 
          ...
          description="Pick words that resonate with you" 
          icon="text" 
          gameId="word_association"
          isSelected={selectedGame === 'word_association'}
          onSelect={setSelectedGame}
        />
        <GameCard 
          title="Mood Slider" 
          description="Slide from calm to adventurous" 
          icon="options" 
          gameId="mood_slider"
          isSelected={selectedGame === 'mood_slider'}
          onSelect={setSelectedGame}
        />
        <GameCard 
          title="Color Palette" 
          ...
          icon="color-palette" 
          gameId="color_picker"
          isSelected={selectedGame === 'color_picker'}
          onSelect={setSelectedGame}
        />
